        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-3xl shadow-2xl w-full max-w-6xl max-h-[90vh] flex flex-col overflow-hidden">
                ...
                <div className="flex border-b border-slate-200 shrink-0">
                    ...
                    >
                        <BookOpen className="w-5 h-5" /> 操作ガイド
                    </button>
                    <button
                        onClick={() => setActiveTab('manual')}
                        className={`flex-1 py-4 text-base font-bold flex items-center justify-center gap-2 transition ${activeTab === 'manual' ? 'text-indigo-600 border-b-2 border-indigo-600 bg-indigo-50' : 'text-slate-500 hover:bg-slate-50'}`}
                    >
                        <FileText className="w-5 h-5" /> 詳細マニュアル
                    </button>
                    <button
                    ...
                    >
                        <HelpCircle className="w-5 h-5" /> よくある質問
                    </button>
                </div>
                ...
                <div className="p-8 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-300 bg-slate-50/50 flex-1">
                    {activeTab === 'usage' ? (
                        ...
                        <div className="max-w-4xl mx-auto bg-white p-10 rounded-3xl border border-slate-200 shadow-sm prose prose-slate prose-indigo">
                            <h2 className="text-3xl font-bold text-slate-800 mb-6 border-b-4 border-indigo-600 pb-2">京都バス退職金試算システム 操作マニュアル</h2>
                            
                            <div className="space-y-8 text-slate-700 leading-relaxed">
                                <section>
                                    <h3 className="text-xl font-bold text-indigo-700 mb-3 flex items-center gap-2">
                                        <CheckCircle className="w-6 h-6"/> 1. 基本構成
                                    </h3>
                                    <p>本システムは、現行の退職金制度（B案）と改定案（A案）を比較し、将来の退職金支給額および引当金の推移をシミュレーションするためのツールです。</p>
                                    <ul className="list-disc list-inside space-y-1 ml-4">
                                        <li><strong>上部</strong>: 全体設定・インポート（シミュレーション期間、社員データ読込）</li>
                                        <li><strong>左側</strong>: パターンA（変更案）設定（定年延長、ポイント表、調整ポイント等）</li>
                                        <li><strong>右側</strong>: パターンB（現行制度）設定（現状のパラメータ）</li>
                                        <li><strong>下部</strong>: シミュレーション結果（グラフ、詳細テーブル、AI分析レポート）</li>
                                    </ul>
                                </section>

                                <section>
                                    <h3 className="text-xl font-bold text-indigo-700 mb-3 flex items-center gap-2">
                                        <CheckCircle className="w-6 h-6"/> 2. 操作手順
                                    </h3>
                                    <div className="space-y-4 ml-4">
                                        <div>
                                            <h4 className="font-bold text-slate-800 underline decoration-indigo-300 underline-offset-4">ステップ1：社員データの読み込み</h4>
                                            <p className="text-sm mt-1">「社員データ読込」ボタンからCSV/Excelを読み込みます。必須項目（社員番号、氏名、生年月日、入社日、資格、累積ポイント）が含まれていることを確認してください。</p>
                                        </div>
                                        <div>
                                            <h4 className="font-bold text-slate-800 underline decoration-indigo-300 underline-offset-4">ステップ2：制度内容の設定</h4>
                                            <p className="text-sm mt-1">左右のパネルで定年年齢や単価を設定します。「ポイント表・係数を編集」ボタンから、年数ごとの付与ポイントや支給率係数を詳細にカスタマイズできます。</p>
                                        </div>
                                        <div>
                                            <h4 className="font-bold text-slate-800 underline decoration-indigo-300 underline-offset-4">ステップ3：シミュレーション実行</h4>
                                            <p className="text-sm mt-1">設定を変更するとリアルタイムで計算されますが、「再計算する」ボタンを押すことで確実に最新の状態でグラフとテーブルが更新されます。</p>
                                        </div>
                                        <div>
                                            <h4 className="font-bold text-slate-800 underline decoration-indigo-300 underline-offset-4">ステップ4：結果の出力</h4>
                                            <p className="text-sm mt-1">「比較結果をExcel出力」ボタンから、全社員の試算結果と年度別コスト推移を保存できます。</p>
                                        </div>
                                    </div>
                                </section>
                                ...
                                <section>
                                    <h3 className="text-xl font-bold text-indigo-700 mb-3 flex items-center gap-2">
                                        <Sparkles className="w-6 h-6 text-purple-600"/> 3. AI分析レポート機能
                                    </h3>
                                    <p>Gemini AIを使用して、複雑な試算結果の分析や制度設計の提案を受けることができます。</p>
                                    <ul className="list-disc list-inside space-y-1 ml-4 text-sm">
                                        <li><strong>APIキーの選択</strong>: Google CloudのAPIキーを安全なダイアログから選択します。</li>
                                        <li><strong>目標設定</strong>: 「コスト削減目標」を有効にすると、目標数値に合わせたパラメータをAIが逆算します。</li>
                                        <li><strong>分析実行</strong>: 財務リスクや不利益変更の有無をAIがチェックします。</li>
                                        <li><strong>提案の反映</strong>: AIの提案を「A案に反映」ボタンで即座にシミュレーションに適用できます。</li>
                                    </ul>
                                </section>

                                <section className="bg-slate-100 p-6 rounded-2xl border border-slate-200">
                                    <h3 className="text-lg font-bold text-slate-800 mb-2 flex items-center gap-2">
                                        <Lock className="w-5 h-5 text-slate-600"/> 注意事項
                                    </h3>
                                    <ul className="text-sm space-y-2 text-slate-600">
                                        <li>・本システムはブラウザ上で動作します。リロードすると設定がリセットされるため、重要な設定はエクスポートして保存してください。</li>
                                        <li>・個人情報はブラウザ内でのみ処理され、外部サーバー（AI送信時を除く）には保存されません。</li>
                                        <li>・AIへの送信データには、氏名などの個人を特定する情報は含まれず、統計データのみが送信されます。</li>
                                    </ul>
                                </section>
                            </div>
                        </div>
